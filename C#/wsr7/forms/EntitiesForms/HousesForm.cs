...
namespace wsr7.Forms.EntitiesForms
{
    public partial class HousesForm : Form
    {
        ...
        {
            grid_Houses.Rows.Clear();
            List<House> houses;
            if (filter == "")
                houses = ModelContext.init().Houses.Include(e => e.Realty).ToList();
            else
                houses = ModelContext.init().Houses.Include(e => e.Realty).AsEnumerable().Where(c => new LengthLevensht.LengthLevensht(c.Realty.Address_City, filter).getItog() <= 3 ||
                    ...
                    ToList();
            foreach (House e in houses)
            {
                int r = grid_Houses.Rows.Add(e.Realty_ID, e.TotalFloors, e.Rooms, e.TotalArea, e.Realty.Address_City, e.Realty.Address_Street, e.Realty.Address_House, e.Realty.Address_Number, e.Realty.Coordinate_latitude, e.Realty.Coordinate_longitude);
                grid_Houses.Rows[r].Tag = e;
            }
        }
        ...
        {
            if (grid_Houses.SelectedRows.Count != 1)
                return;
            if (!ReallyDoIt())
                return;
            House cl = grid_Houses.SelectedRows[0].Tag as House;
        ...
        private void But_Add_Click(object sender, EventArgs e)
        {
            ...
            ModelContext.init().Houses.Add(ear);
            save();
        }
        ...
        private void grid_Houses_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (grid_Houses.SelectedRows.Count != 1)
                return;
            House cl = grid_Houses.SelectedRows[0].Tag as House;
            fie_edit_area.Text = cl.TotalArea.ToString();
            fie_edit_floor.Text = cl.TotalFloors.ToString();
